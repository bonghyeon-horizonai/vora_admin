...
export default function ProductListTable({
    ...
    const handleDelete = async () => {
        ...
        startTransition(async () => {
            ...
            if (result.success) {
                ...
                enqueueSnackbar(result.error || 'Failed to delete product', { variant: 'error' });
            }
            setDeleteId(null);
        });
    };
    ...
    const columns: GridColDef[] = [
        ...
        },
        {
            ...
            renderCell: (params: GridRenderCellParams) => (
                <Box sx={{ display: 'flex', alignItems: 'center', height: '100%' }}>
                    <Typography variant="body2">
                        {params.value === 'MONTHLY' ? t('products-period-monthly') :
                            params.value === 'YEARLY' ? t('products-period-yearly') :
                                params.value === 'ONCE' ? t('products-period-once') :
                                    params.value || '-'}
                    </Typography>
                </Box>
            ),
        },
        {
            ...
            renderCell: (params: GridRenderCellParams) => (
                <Box sx={{ display: 'flex', alignItems: 'center', height: '100%' }}>
                    <Chip
                        label={params.value ? t('products-status-active') : t('products-status-inactive')}
                        size="small"
    ...
    const sortModel: GridSortModel = sortBy ? [{ field: sortBy, sort: sortOrder || 'desc' }] : [];
    ...
    useEffect(() => {
        ...
        setPaginationModel((current) => {
            if (current.page !== currentPageInUrl - 1 || current.pageSize !== pageSize) {
