...
export async function searchToolsAction(
  ...
  const results = await db
    ...
    .leftJoin(
      targetName,
      and(
        eq(tools.id, targetName.toolId),
        eq(targetName.languageCode, dbLocale as any),
      ),
    )
    .leftJoin(
      krName,
      and(eq(tools.id, krName.toolId), eq(krName.languageCode, "KR")),
    )
    .leftJoin(
      enName,
      and(eq(tools.id, enName.toolId), eq(enName.languageCode, "EN")),
    )
    .where(
      query
        ? or(
 ...
 */
export async function syncPaddleProductAction(productId: string): Promise<{
  success: boolean;
  error?: string;
  ...
  try {
    ...
    const productName =
      enI18n?.name || product.productCode || "Unnamed Product";
