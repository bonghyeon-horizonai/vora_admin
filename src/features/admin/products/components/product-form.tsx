...
export default function ProductForm({ product, isNew }: ProductFormProps) {
  ...
  const [toolSearchResults, setToolSearchResults] = useState<
    { id: string; toolCode: string | null; name: string }[]
  >([]);
  ...
  const defaultValues: CreateProductSchema = {
    ...
    billingCycle:
      (product?.billingCycle as "MONTHLY" | "YEARLY" | "ONCE") || "MONTHLY",
  ...
  const onSyncPaddle = async () => {
    if (isNew || !product?.id) {
      enqueueSnackbar(
        "Please save the product first before syncing with Paddle",
        { variant: "info" },
      );
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        enqueueSnackbar("Synced with Paddle successfully", {
          variant: "success",
        });
        if (result.paddleProductId)
          setValue("paddleProductId", result.paddleProductId);
        if (result.paddlePriceId)
          setValue("paddlePriceId", result.paddlePriceId);
      } else {
        ...
        <Grid container spacing={3}>
                ...
                <FormControl fullWidth>
                  ...
                  <Select {...field} label={t("products-form-section-type")}>
                    <MenuItem value="SUBSCRIPTION">
                      {t("products-form-type-subscription")}
                    </MenuItem>
                    <MenuItem value="PURCHASE">
                      {t("products-form-type-purchase")}
                    </MenuItem>
                  </Select>
                </FormControl>
              )}
          ...
          <Grid size={{ xs: 12, md: 6 }}>
            <Controller
              ...
              render={({ field }) => (
                <TextField
                  {...field}
                  label={t("products-form-paddle-id")}
                  fullWidth
                  value={field.value || ""}
                />
              )}
          ...
          <Grid size={{ xs: 12, md: 6 }}>
            <Controller
              ...
              render={({ field }) => (
                <TextField
                  {...field}
                  label={t("products-form-paddle-price-id")}
                  fullWidth
                  value={field.value || ""}
                />
              )}
          ...
          <Grid size={{ xs: 12 }}>
            ...
            >
              {isSyncing
                ? "Syncing..."
                : "Sync with Paddle API (Auto-generate IDs)"}
            </Button>
            {isNew && (
              <Typography
                variant="caption"
          ...
          <Grid size={{ xs: 12 }}>
            ...
            {LANGUAGES.map((lang, idx) => (
              <Box
                key={lang.code}
                sx={{ display: langTab === idx ? "block" : "none" }}
              >
          ...
          <Grid size={{ xs: 12 }}>
            ...
            <Autocomplete
              ...
              getOptionLabel={(option) =>
                `[${option.toolCode}] ${option.name || "No Name"}`
              }
              ...
              renderInput={(params) => (
                <TextField
                  {...params}
                  label={t("products-form-tools-search")}
                  fullWidth
                />
              )}
            />
            ...
            <Stack spacing={1}>
              {toolFields.map((field, index) => {
                ...
                const toolObj = product?.tools.find(
                  (t) => t.toolId === field.toolId,
                );
                const selectedResult = toolSearchResults.find(
                  (r) => r.id === field.toolId,
                );
                const displayName =
                  toolObj?.name || selectedResult?.name || "No Name";
                const displayCode =
                  toolObj?.toolCode || selectedResult?.toolCode || "";
          ...
          <Grid size={{ xs: 12 }}>
            <Controller
              ...
              render={({ field }) => (
                <TextField
                  {...field}
                  label={t("products-form-icon-url")}
                  fullWidth
                  value={field.value || ""}
                  placeholder="https://..."
                />
