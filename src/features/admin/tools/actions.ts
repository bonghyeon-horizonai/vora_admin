...
import { toolI18N, tools } from "@/lib/db/schema";
...
export async function updateToolAction(
  ...
  try {
    ...
    await db.transaction(async (tx) => {
      ...
      if (toolData.toolCode !== undefined)
        updateData.toolCode = toolData.toolCode;
      if (toolData.internalUsageLimit !== undefined)
        updateData.internalUsageLimit = toolData.internalUsageLimit;
      if (toolData.isFree !== undefined) updateData.isFree = toolData.isFree;
      if (toolData.tier !== undefined) updateData.tier = toolData.tier;
      if (toolData.iconImageUrl !== undefined)
        updateData.iconImageUrl = toolData.iconImageUrl || null;
      if (toolData.isActive !== undefined)
        updateData.isActive = toolData.isActive;
