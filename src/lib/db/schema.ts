...
export const agentPromptStatusEnum = pgEnum("agent_prompt_status_enum", [
  "PENDING",
  "ACCEPTED",
  "REJECTED",
]);
...
export const billingProviderEnum = pgEnum("billing_provider_enum", [
  "PADDLE",
  "STRIPE",
]);
export const conversationModeEnum = pgEnum("conversation_mode_enum", [
  "GENERAL",
  "PDCA",
  "DISCUSSION",
]);
...
export const fileProviderEnum = pgEnum("file_provider_enum", [
  "GROK",
  "S3",
  "LOCAL",
  "AZURE",
...
export const languageCodeEnum = pgEnum("language_code_enum", [
  "KR",
  "EN",
  "JP",
]);
export const logMethodEnum = pgEnum("log_method_enum", [
  "POST",
  "GET",
  "PUT",
  "DELETE",
...
export const toolCategoryEnum = pgEnum("tool_category_enum", [
  "SEARCH",
  "GENERATION",
  "OPERATION",
  "DOCUMENTS",
...
export const workflowTypeEnum = pgEnum("workflow_type_enum", [
  "MAIN",
  "PARTICIPANT-A",
  "PARTICIPANT-B",
  "ROLE-SETTER",
...
export const sessions = pgTable(
  ...
  (table) => [
    uniqueIndex("idx_sessions_token").using(
      "btree",
...
export const invitations = pgTable(
  ...
  (table) => [
    uniqueIndex("idx_invitations_invited_user_id").using(
      "btree",
      table.invitedUserId.asc().nullsLast().op("uuid_ops"),
    ),
    uniqueIndex("idx_invitations_token").using(
      "btree",
      table.token.asc().nullsLast().op("text_ops"),
    ),
    index("idx_invitations_user_id").using(
      "btree",
      table.userId.asc().nullsLast().op("uuid_ops"),
    ),
    foreignKey({
...
export const agents = pgTable(
  ...
  (table) => [
    index("idx_agents_active_prompt_id").using(
      "btree",
      table.activeAgentPromptHistoryId.asc().nullsLast().op("uuid_ops"),
    ),
    index("idx_agents_user_id").using(
      "btree",
      table.userId.asc().nullsLast().op("uuid_ops"),
    ),
    foreignKey({
...
export const agentPromptHistories = pgTable(
  "agent_prompt_histories",
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
  ...
  (table) => [
    index("idx_prompt_histories_agent_id").using(
      "btree",
...
export const conversationParticipants = pgTable(
  ...
  (table) => [
    index("idx_participants_agent_id").using(
      "btree",
      table.agentId.asc().nullsLast().op("uuid_ops"),
    ),
    index("idx_participants_conv_id").using(
      "btree",
      table.conversationId.asc().nullsLast().op("uuid_ops"),
    ),
    index("idx_participants_user_id").using(
      "btree",
      table.userId.asc().nullsLast().op("uuid_ops"),
    ),
    foreignKey({
...
export const messageShares = pgTable(
  "message_shares",
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
    conversationId: uuid("conversation_id").notNull(),
...
export const messages = pgTable(
  ...
  (table) => [
    index("idx_messages_agent_id").using(
      "btree",
      table.agentId.asc().nullsLast().op("uuid_ops"),
    ),
    ...
    index("idx_messages_conv_id").using(
      "btree",
      table.conversationId.asc().nullsLast().op("uuid_ops"),
    ),
    index("idx_messages_question_trgm").using(
      "gin",
      table.question.asc().nullsLast().op("gin_trgm_ops"),
    ),
    index("idx_messages_user_id").using(
      "btree",
      table.userId.asc().nullsLast().op("uuid_ops"),
    ),
    index("idx_messages_workflows_id").using(
      "btree",
      table.workflowsId.asc().nullsLast().op("uuid_ops"),
    ),
    foreignKey({
      columns: [table.agentId],
      foreignColumns: [agents.id],
...
export const files = pgTable(
  ...
  (table) => [
    index("idx_files_agent_id").using(
      "btree",
      table.agentId.asc().nullsLast().op("uuid_ops"),
    ),
    index("idx_files_user_id").using(
      "btree",
      table.userId.asc().nullsLast().op("uuid_ops"),
    ),
    foreignKey({
...
export const userPaymentInfos = pgTable(
  "user_payment_infos",
  {
...
export const conversationArtifacts = pgTable(
  ...
  (table) => [
    index("idx_artifacts_message_id").using(
      "btree",
...
export const toolI18N = pgTable(
  ...
  (table) => [
    index("idx_tool_i18n_tool_id").using(
      "btree",
      table.toolId.asc().nullsLast().op("uuid_ops"),
    ),
    foreignKey({
...
export const productTools = pgTable(
  ...
  (table) => [
    index("idx_product_tools_product_id").using(
      "btree",
      table.productId.asc().nullsLast().op("uuid_ops"),
    ),
    index("idx_product_tools_tool_id").using(
      "btree",
      table.toolId.asc().nullsLast().op("uuid_ops"),
    ),
    foreignKey({
...
export const messageFiles = pgTable(
  ...
  (table) => [
    ...
    index("idx_message_files_message_id").using(
      "btree",
      table.messageId.asc().nullsLast().op("uuid_ops"),
    ),
    foreignKey({
...
export const productsI18N = pgTable(
  "products_i18n",
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
    productId: uuid("product_id").notNull(),
  ...
  (table) => [
    index("idx_product_i18n_product_id").using(
      "btree",
      table.productId.asc().nullsLast().op("uuid_ops"),
    ),
    foreignKey({
...
export const userPurchases = pgTable(
  ...
  (table) => [
    index("idx_purchase_product_id").using(
      "btree",
      table.productId.asc().nullsLast().op("uuid_ops"),
    ),
    index("idx_purchase_user_id").using(
      "btree",
      table.userId.asc().nullsLast().op("uuid_ops"),
    ),
    foreignKey({
...
export const purchaseToolQuotas = pgTable(
  "purchase_tool_quotas",
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
  ...
  (table) => [
    index("idx_quotas_license_id").using(
      "btree",
      table.licenseId.asc().nullsLast().op("uuid_ops"),
    ),
    index("idx_quotas_tool_id").using(
      "btree",
      table.toolId.asc().nullsLast().op("uuid_ops"),
    ),
    foreignKey({
...
export const purchaseToolUsageLogs = pgTable(
  "purchase_tool_usage_logs",
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
    type: text(),
  ...
  (table) => [
    index("idx_usage_logs_quota_id").using(
      "btree",
      table.quotaId.asc().nullsLast().op("uuid_ops"),
    ),
    index("idx_usage_logs_tool_id").using(
      "btree",
      table.toolId.asc().nullsLast().op("uuid_ops"),
    ),
    index("idx_usage_logs_user_id").using(
      "btree",
      table.extraUserId.asc().nullsLast().op("uuid_ops"),
    ),
    foreignKey({
      columns: [table.conversationId],
      foreignColumns: [conversations.id],
...
export const automationTasks = pgTable(
  ...
  (table) => [
    index("idx_automation_tasks_agent_id").using(
      "btree",
      table.agentId.asc().nullsLast().op("uuid_ops"),
    ),
    foreignKey({
...
export const taskExecutionLogs = pgTable(
  ...
  (table) => [
    index("idx_execution_logs_task_id").using(
      "btree",
...
export const userFriends = pgTable(
  ...
  (table) => [
    uniqueIndex("idx_user_friends_pair").using(
      "btree",
      table.userId.asc().nullsLast().op("uuid_ops"),
      table.friendUserId.asc().nullsLast().op("uuid_ops"),
    ),
    foreignKey({
...
export const logErrors = pgTable(
  ...
  (table) => [
    index("idx_log_errors_user_id").using(
      "btree",
      table.userId.asc().nullsLast().op("uuid_ops"),
    ),
    foreignKey({
...
export const ragUserRelations = pgTable(
  "rag_user_relations",
  {
    userId: uuid("user_id").notNull(),
  ...
  (table) => [
    index("idx_rag_relations_key_id").using(
      "btree",
      table.keyId.asc().nullsLast().op("uuid_ops"),
    ),
    foreignKey({
      columns: [table.keyId],
    ...
    primaryKey({
      columns: [table.keyId, table.userId],
      name: "rag_user_relations_pkey",
    }),
  ],
);
